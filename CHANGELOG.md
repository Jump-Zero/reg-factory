...
**Codex K12 新实现**
- 新建独立 `k12/` Python/FastAPI 服务与原生前端，不复用旧 `codex_k12` 应用代码或数据模型。
- K12 保持独立端口 `8806`、独立配置、独立任务队列和独立 `runtime/k12` 数据目录；主 WebUI 只负责检测、启动、停止和打开入口。
- 复用项目已有邮箱解析、浏览器、OAuth、Sub2API、代理和任务执行基础能力，支持邮箱导入、Workspace request/accept、批量任务、取消和日志查看。
- 新增独立启动脚本 `start_k12.bat` / `start_k12.sh`。
...
- K12 服务健康检查、配置读写、邮箱导入和静态前端接口验证通过。
- WebUI JavaScript 语法检查通过。

**增强验证**
- K12 健康、摘要、配置、邮箱、任务和数据 API 验证通过。
- ZIP 完整性测试通过。
